...
class Method_CNN(method, nn.Module):
    ...
    def __init__(self, mName, mDescription):
        ...
        # Block 3
        # We take the 64 channels from block 2 and maybe output 128
        ...
        # Default fits MNIST (28x28 -> after 2 pools -> 7x7 -> 32*7*7=1568)
    ...
    def train_model(self, X, y):
        ...
        # 3. Create One-Hot labels for MSE
        # Converts [5] -> [0, 0, 0, 0, 0, 1, 0, ...]
        y_one_hot = torch.nn.functional.one_hot(y_tensor, num_classes=self.num_classes).float()
        ...
        for epoch in range(self.max_epoch):
            ...
            # 4. Calculate Loss using the One-Hot labels
            ...
            if epoch % 10 == 0:
                # For accuracy, we still use the original y_tensor (integers)
                accuracy_evaluator.data = {'true_y': y_tensor, 'pred_y': y_pred.max(1)[1]}
                print(f'Epoch: {epoch} | Accuracy: {accuracy_evaluator.evaluate():.4f} | Loss: {train_loss.item():.4f}')
